...
const WEB_APP_URL = 'https://script.google.com/macros/s/REEMPLAZAR_CON_ID_DE_DESPLIEGUE/exec';
...
function bindFormulario() {
  $('#form-vak').addEventListener('input', (e) => {
    ...
    if (campo === 'nombre' && !STATE.form.correo) {
      // autocompletar sugerencia de correo institucional
    }
  ...
  $('#nombre').addEventListener('blur', () => {
    if (STATE.form.nombre && !STATE.form.correo) {
      const slug = STATE.form.nombre.trim().toLowerCase()
...
function renderResultado(res) {
  ...
  $('#tabla-yo').innerHTML = `
    ...
    <tr><th>Por lo tanto debo...</th><td>${info.recomendacion}</td></tr>
  `;
